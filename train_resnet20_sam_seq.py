...
if args.train:
    ...
    for rho in rhos:
        ...
        model = model_seq.create_model('ResNet20').to(device)

        #sam
        base_optimizer = torch.optim.SGD
        optimizer = SAM(model.parameters(), base_optimizer, rho=rho ,lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
        ...
        log_dict = {'train_loss': train_loss_log, 'test_loss': test_loss_log,
                                ...
                                'seed': args.seed, 'gamma': args.gamma, 'scheduler': 'paper', 'rho': rho}
        with open(f'train_resnet20_sam{rho}.json', 'w') as outfile:
