...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        loss = criterion(outputs, targets) #loss here is used for training statistics
        loss.backward()
        optimizer.first_step(zero_grad=True) 
        
        train_loss += loss.item() #compute loss for statistical purpose
...
def test_in_train(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/lenet_sam_ckpt.pth')
        best_acc = acc
...
if args.train:
    ...
    if not os.path.isdir('checkpoint'):
        os.mkdir('checkpoint')
    torch.save(state, './checkpoint/lenet_sam_final_ckpt.pth')
