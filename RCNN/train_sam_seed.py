...
if __name__=='__main__':
    ...
    for seed in seeds:
        ...
        optimizer = SAM(model.parameters(), base_optimizer, rho=config.rho, lr=config.lr, momentum=config.momentum)
        NLLLoss = nn.NLLLoss()
        ...
        log_dict = {'train_loss': train_losses, 'test_loss': test_losses, 'val_loss': val_losses, 'val_acc': val_accuracies,
                                ...
                                'dropout': config.dropout_keep, 'momentum': config.momentum, 'seed': seed, 'rho': config.rho }
        with open(f'train_rcnn_sam{seed}_{config.rho}.json', 'w') as outfile:
